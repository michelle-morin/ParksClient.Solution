...
namespace ParksClient.Models
{
  class ApiHelper
  {
    ...
    public static async Task<string> Search(string name, string agency, string state, bool? campsites)
    {
      RestClient client = new RestClient("http://localhost:5000/api");
      RestRequest request = new RestRequest($"parks?name={name}&agency={agency}&state={state}&campsites={campsites}", Method.GET);
      var response = await client.ExecuteTaskAsync(request);
      return response.Content;
    }
